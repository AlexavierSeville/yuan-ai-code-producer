...
import com.yuan.yuanaicodeproducer.config.ConcurrentChatLimiterConfig;
...
public class AppController {
    ...
    private final ConcurrentChatLimiterConfig concurrentChatLimiterConfig;
    ...
    public Flux<ServerSentEvent<String>> chatToGenCode(
        ...
        // 检查并发限制
        if (!concurrentChatLimiterConfig.tryAcquire()) {
            log.warn("系统繁忙，当前活跃对话数: {}/3", concurrentChatLimiterConfig.getActiveChats());
            return Flux.just(ServerSentEvent.<String>builder()
                    .event("error")
                    .data("系统繁忙，当前有 " + concurrentChatLimiterConfig.getActiveChats() + " 个对话正在进行，请稍后再试")
                    .build());
        }
        ...
        return contentFlux
                ...
                .doFinally(signalType -> {
                    // 确保释放并发许可
                    concurrentChatLimiterConfig.release();
                    log.info("对话结束，释放并发许可，当前活跃对话数: {}/3", concurrentChatLimiterConfig.getActiveChats());
                })
                .map(chunk -> {
